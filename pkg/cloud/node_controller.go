...
func (cnc *CloudNodeController) enqueue(obj interface{}) {
	key, err := cache.DeletionHandlingMetaNamespaceKeyFunc(obj)
	if err != nil {
		utilruntime.HandleError(err)
		return
	}
	klog.Infof("New node %s added, syncing", key)
	cnc.workqueue.AddRateLimited(obj)
}

func (cnc *CloudNodeController) updateNode(_, cur interface{}) {
	key, err := cache.DeletionHandlingMetaNamespaceKeyFunc(cur)
	if err != nil {
		utilruntime.HandleError(err)
		return
	}
	klog.Infof("Node %s spec updated, syncing", key)
	cnc.workqueue.AddRateLimited(cur)
}
...
func (cnc *CloudNodeController) Run(stopCh <-chan struct{}) {
	...
	workers, err := strconv.Atoi(os.Getenv("NODE_WORKERS"))
	if err != nil {
		klog.Errorf("failed to get node worker from env: %v", err)
	}
	klog.Infof("NODE WORKERS from env is %d", workers)
	if workers == 0 {
		workers = 1
	}
	klog.Infof("Node controller using worker is %d", workers)
...
func (cnc *CloudNodeController) initializeNode(node *v1.Node) {
	...
	err := clientretry.RetryOnConflict(UpdateNodeSpecBackoff, func() error {
		...
		ReqID := cloud_provider.GetRandom()
		...
		klog.Infof("ReqID: %s. Before update node is %s %v", ReqID, curNode.Name, curNode.ResourceVersion)
		...
		klog.Infof("ReqID: %s. After exclude taint node is %s %v", ReqID, curNode.Name, curNode.ResourceVersion)

		newestNode, err := cnc.kubeClient.CoreV1().Nodes().Get(node.Name, metav1.GetOptions{})
		if err != nil {
			return err
		}
		klog.Infof("ReqID: %s. Newest node resource version is %v", ReqID, newestNode.ResourceVersion)
		curNode.ResourceVersion = newestNode.ResourceVersion
